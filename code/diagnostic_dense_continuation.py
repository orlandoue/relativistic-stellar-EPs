"""
diagnostic_dense_continuation.py — DIAGNOSTICO de la continuacion densa
(NO genera la Figura 4 del manuscrito; esa la produce bordered_newton_robustness.py)
=========================================================================
...
Sistema: seccion 7 con perfil deformado C = 1 + eta (1 - sigma^2)   [Ec. (49)]
...
if __name__ == "__main__":
    ...
    # ---- diagnostico: donde se detiene cada resolucion
    # NOTA: este script NO calcula eta_c. La extrapolacion de pliegue a partir
    # de la continuacion en eta es inestable, porque para algunas resoluciones
    # la corrida se detiene en el GIRO (eta ~ 0.11) y no en el pliegue, de modo
    # que el ajuste mezcla dos fenomenos distintos y devuelve valores sin
    # sentido. El valor publicado, eta_c = 0.1941(1), procede de
    # bordered_newton_robustness.py, que continua con control de paso y
    # rechazo de salto de rama hasta agotar el paso en el pliegue.
    print("\nUltimo punto alcanzado por cada resolucion:")
    for N, rows in data.items():
        e_last, s_last, z_last, r_last = rows[-1]
        tipo = "giro (~0.11)" if float(e_last) < 0.15 else "cerca del pliegue"
        print(f"  N={N}: eta={float(e_last):.5f}  zeta_0*={float(z_last):.6f}"
              f"  [{tipo}]")
    print("  eta_c NO se estima aqui; ver bordered_newton_robustness.py")
    ...
    ec = 0.1941   # valor publicado (bordered_newton_robustness.py)
    for ax in (a1, a2):
        ax.axvline(ec, color='red', ls=':', lw=1.2)
    a1.annotate(rf'$\eta_c={ec:.4f}$ (ref.)', (ec, max(zz)), color='red',
                fontsize=8, ha='right')
    ...
    out = os.path.join(outdir, 'diagnostic_dense_continuation.pdf')
    fig.savefig(out, dpi=150, bbox_inches='tight')
    print(f'\ndiagnostico guardado en {out}')
    print('  (la Figura 4 del manuscrito la genera bordered_newton_robustness.py)')
